...
class Dataset_TIP(torch.utils.data.Dataset):
    # test on the new 12 textures
    def __init__(self, data_dir, label_file, dist = None, ref = 'original', format = '.png'):
        self.ref_dir = os.path.join(data_dir, ref)
        self.label_file = os.path.join(data_dir, label_file)
        self.labels = self._getlabels()
        ...
        self.dist_img_paths = self.dist_img_paths[:-9]
    ...
    def _getlabels(self):
        ...
        '''
        df = pd.read_excel(self.label_file, header=None)
        label1 = df.iloc[1:,2:].to_numpy().astype(np.double)
...
class Dataset_TIP_all(torch.utils.data.Dataset):
    # 22 texture
    def __init__(self, data_dir, label_file, dist = None, ref = 'original', format = '.png'):
        self.ref_dir = os.path.join(data_dir, ref)
        self.label_file = os.path.join(data_dir, label_file)
    ...
    def __getitem__(self, item):
        ...
        return ref_img, dist_img, y, int(t), 0  # the last item is useless
...
if __name__ == "__main__":
    ...
    image_dir = '/dataset/new_data_TIP_all/'
    label_file = 'labels_global_v3.xlsx'
    dist_img_folder = '5-fold_cv/train_0.json'
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    dataset = Dataset_TIP_all(data_dir=image_dir, label_file=label_file, dist=dist_img_folder)
    # dataset = Dataset(data_dir=image_dir, label_file=label_file, dist=dist_img_folder)
    # dataset = Dataset_TIP(data_dir=image_dir, label_file=label_file, dist=dist_img_folder)

    batch_size = 10000  # the actually batchsize <= total images in dataset
    data_generator = torch.utils.data.DataLoader(dataset, batch_size=batch_size)

    for X1, X2, Y, _ in data_generator:
