...
def plot_psds(psd1, psd2, psd3, figname='tmp1.png', normalize=True):
    if normalize:
        w_dis1 = wasserstein_distance(psd1 / np.sum(psd1), psd2 / np.sum(psd2))
        w_dis2 = wasserstein_distance(psd1 / np.sum(psd1), psd3 / np.sum(psd3))
    else:
        w_dis1 = wasserstein_distance(psd1, psd2)
        w_dis2 = wasserstein_distance(psd1, psd3)

    k = 1
    plt.plot(range(k,psd1.size), psd1[k:], label='original')
    plt.plot(range(k,psd2.size), psd2[k:], label='av1_synthesized')
    plt.plot(range(k,psd3.size), psd3[k:], label='H&B_synthesized')
    plt.legend()
    plt.title('wasserstein distance={:0.5f}, {:0.5f}'.format(w_dis1, w_dis2))
    #plt.show()
    plt.yscale('log')
    plt.savefig(figname)
    plt.close()
...
img_den = 'data/brooklyn_nine_nine/denoised/frame_00001.png'
img_o = 'data/brooklyn_nine_nine/original/frame_00001.png'
psd1d_o = main(img_o, img_den, 'original film grain', psd1d_o=None, viz=False)

img_dec = 'data/brooklyn_nine_nine/decoded/frame_00001.png'
img_ren = 'data/brooklyn_nine_nine/renoised/frame_00001.png'
psd1d_syn_av1 = main(img_ren, img_dec, 'av1 synthesized film grain', psd1d_o=None, viz=False)

fg = 'data/brooklyn_nine_nine/fg_synthesized_scaled.png'
psd1d_syn_HB = main1(fg, 'H&B synthesized film grain', psd1d_o, viz=False)
