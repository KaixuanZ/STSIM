...
from utils.dataset_KADID10k import Dataset
...
if __name__ == '__main__':
    ...
    # parser.add_argument("--config", type=str, default="config/train_STSIM_global_KADID10k.cfg", help="path to data config file")
    parser.add_argument("--config", type=str, default="config/train_DISTS_global_KADID10k.cfg", help="path to data config file")
    ...
    trainset = Dataset(data_dir=dataset_dir, label_file=label_file, data_split='train')
    ...
    validset = Dataset(data_dir=dataset_dir, label_file=label_file, data_split='valid')
    ...
    if config['model'] == 'STSIM':
        ...
        model = STSIM_VGG(config['dim'], grayscale=False).to(device)
    ...
    for i in range(epochs):
        running_loss = []
        for X1_train, X2_train, Y_train, _ in tqdm(train_loader):
