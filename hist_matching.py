...
from utils.histogram_matching import my_match_histograms
...
def steerable_hist_match(ref, image):
    ...
    beta, loc, scale = gennorm.fit(ref)
    plt.plot(x, gennorm.pdf(x, beta, loc, scale)*3.5)

    plt.hist(ref.flatten(), bins='auto', density=1)

    # plt.show()
    plt.savefig('tmp_0514/original.png')
    plt.close()
    import pdb;pdb.set_trace()
    '''
    return image
